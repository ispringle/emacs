(use-package avy
  ...
  ;;             (defun isp/avy-with-single-candidate-jump (orig-fn &optional arg)
  ...
  (defun isp/avy-goto-char-this-window (&optional arg)
  ...
  (defun isp/avy-isearch (&optional arg)
  ...
  (defun isp/avy--read-char-2 (char1 char2)
                       ...
                       (let ((c2 (read-char "char 2: " t)))
                         (cond ((eq c2 ?�)
                               ...
                               ((memq c2 '(8 127))
                                ...
                                (call-interactively 'isp/avy-next-char-2))
                               (t
                                c2)))))
  ...
  (defun isp/avy-next-char-2 (&optional str2 arg)
    "Go to the next occurrence of two characters"
    (interactive (list
                  (call-interactively 'isp/avy--read-char-2)
                  current-prefix-arg))
    ...
    (set-transient-map
     (let ((map (make-sparse-keymap)))
       (define-key map (kbd ";") (lambda (&optional arg) (interactive)
                                   (isp/avy-next-char-2 str2 arg)))
       (define-key map (kbd ",") (lambda (&optional arg) (interactive)
                                   (isp/avy-previous-char-2 str2 arg)))
       map)))

  (defun isp/avy-previous-char-2 (&optional str2 arg)
    "Go to the next occurrence of two characters"
    (interactive (list
                  (call-interactively 'isp/avy--read-char-2)
                  current-prefix-arg))
  ...
  (defun isp/avy-copy-line-no-prompt (arg)
    (interactive "p")
    (avy-copy-line arg)
    (beginning-of-line)
    (zap-to-char 1 32)
  ...
  (defun isp/avy-link-hint (&optional win)
...
The current window is chosen if WIN is not specified." 
  ...
  (advice-add 'avy-jump :filter-args
              (defun isp/avy-jump-parens (args)
                (let ((new-regex
                       (isp/avy-replace-syntax-class (car args))))
                  (cons new-regex (cdr args)))))

  (defun isp/avy-replace-syntax-class (regex)
    (thread-last regex
                 (string-replace "\\[" "\\s(")
                 (string-replace "\\]" "\\s)")
                 (string-replace ";" "\\(?:;\\|:\\)")
  ...
  (defun isp/avy-goto-char-timer (&optional arg)
    ...
    (let ((avy-all-windows (if arg
      ...
      (avy-with avy-goto-char-timer
        (setq avy--old-cands (avy--read-candidates
                              (lambda (str) (isp/avy-replace-syntax-class
                                             (regexp-quote str)))))
        (avy-process avy--old-cands))))
  
  :general
  ("C-M-'"      'avy-resume
   "C-'"        '(isp/avy-goto-char-this-window :wk "Avy goto char")
   "M-j"        '(isp/avy-goto-char-timer       :wk "Avy goto char timer")
   ...
   "M-s s"      '(isp/avy-next-char-2           :wk "Avy snipe forward")
   "M-s r"      '(isp/avy-previous-char-2       :wk "Avy snipe backward")
   "M-g l"      '(avy-goto-end-of-line         :wk "Avy goto line")
   "M-s z"      '(isp/avy-copy-line-no-prompt   :wk "Avy copy and zap"))
